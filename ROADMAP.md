...
- [~] **H2. 합의(Raft)(28편) + 지속성(29편)** — `src/raft.c`: 리더 선출 + 로그 복제 + §5 안전성 5종(Election/Leader Append/Log Matching/Leader Completeness/State Machine Safety). 결정적 시뮬레이션 네트워크로 검증. 29편: 지속 상태(currentTerm/votedFor/log)를 디스크에 fsync 저장·복구(§5.1) — 실제 크래시 라운드트립으로 이중 투표 방지 증명. test_raft 37 checks. 스냅샷(§7)/멤버십 변경(§6)/선형화 읽기는 프론티어
