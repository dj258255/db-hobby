...
- [~] **H2. 합의(Raft)(28편) + 지속성(29편) + 스냅샷(30편)** — `src/raft.c`: 리더 선출 + 로그 복제 + §5 안전성 5종. 결정적 시뮬레이션 네트워크로 검증. 29편: 지속 상태를 디스크에 fsync(§5.1, 이중 투표 방지 증명). 30편: 로그 압축(§7) — log_base 오프셋 도입, InstallSnapshot으로 뒤처진 팔로워 따라잡기. test_raft 51 checks. 멤버십 변경(§6)/선형화 읽기/엔진 통합은 프론티어
