...
- [ ] **D2. B+Tree latch crabbing(lock coupling)** — 루트->리프로 내려가며 안전하면 조상 latch 조기 해제. 엔진 latch를 걷어내는 다음 계층
...
## 트랙 F — 옵티마이저 (규칙 기반 -> 비용 기반) — **단일 테이블 도달(21편)**

규칙 기반은 "PK 조건이면 무조건 인덱스"였다 — 넓은 범위엔 행마다 랜덤 힙 페치라 순차보다 비싸다.
ANALYZE로 통계를 재고 선택도로 매칭 행 수를 추정해 비용으로 고른다. 조인 순서(F3)는 다음.
...
- [ ] **F3. 비용 기반 조인 순서** — System R식 DP로 N-way 조인 순서+방법 조합 비용 최소화. 지금 "선언 순서대로 왼쪽부터"를 진짜 탐색으로. 히스토그램(균등분포 가정 넘어서기)도 여기서
