...
- [x] **D7. 병렬 집계 + silent 절단 버그 수정 (38편)** — 직렬 exec_select_project는 행을 SELECT_MAX_ROWS(4096)까지만 모아 큰 테이블 집계를 조용히 절단했다(6000행 COUNT(*)=4096 버그). 38편은 병렬 수집으로 이를 고침: 큰 테이블+서브쿼리 없는 집계/GROUP BY에서 워커가 가시성+WHERE 병렬 평가, leader가 매칭 수만큼(cap 없이) rows 채워 기존 aggregate_rowset(직렬과 같은 코드)로 넘김 → cap 없이 정답+형식 동일. 수학 oracle(SUM=n(n+1)/2 등)·MVCC·GROUP BY로 검증, **엔진 ThreadSanitizer 클린**(test_paragg/_tsan). ※ 행을 다 모으므로 메모리 O(n) — 누적만 하는 진짜 partial aggregation은 프론티어
- [x] **D8. 진짜 병렬 부분 집계 — 메모리 O(1) (39편)** — 38편은 매칭 행을 다 모았지만(O(n)), 39편은 워커가 항목별 부분합(total/cnt/sum/min/max)만 누적하고 leader가 결합(Finalize). parscan_foreach 프리미티브(워커별 전용 컨텍스트, 락 없음) 추가. 그룹/HAVING/DISTINCT 없는 순수 INT 집계·서브쿼리 없는 WHERE·큰 테이블만; TEXT MIN/MAX·GROUP BY는 38편/직렬 폴백. 헤더+emit_out_rows 재사용으로 형식 동일. NULL 처리·다중 집계·빈 결과를 수학 oracle로 검증, **ThreadSanitizer 클린**(test_partagg/_tsan). PostgreSQL Partial→Finalize Aggregate 계열. ※ 그룹별 부분 집계(per-group 해시 병합)는 프론티어
- [x] **D9. 병렬 성과 실측 (40편)** — 36~39편이 진짜 빨라졌나 `make bench-parallel`로 측정(워커 1·2·4·8, db_set_parallel_workers 런타임 설정). 12코어 실측: 워밍 CPU-bound에서 per-row 무거울수록 speedup↑(무거운 다중집계+WHERE 4워커 2.16x, COUNT+WHERE 1.98x), 2~4워커 정점 후 하락. 콜드(>풀64프레임)는 풀 latch가 pager_read 직렬화 → 8워커 0.65x. 천장은 CPU가 아니라 공유 latch. test_parworkers(+5)로 워커 수 무관 결과 바이트 동일 검증. → 세밀한 풀·engine_mtx 제거 동기를 숫자로
- [x] **D10. 읽기 I/O를 풀 latch 밖으로 (41편)** — 40편이 잰 콜드 병목(풀 latch가 pager_read 직렬화)을 bufpool.c 주석이 지목한 read-in-progress로 닫음. miss 시 프레임을 io_pending+pin으로 예약하고 latch 놓고 pager_read → 병렬 I/O, 같은 페이지 동시 miss는 io_cond 대기로 중복 로드 방지. 같은 프로세스 A/B(bufpool_set_io_in_latch)로 콜드 SUM 4워커 1.08→2.39x·8워커 0.62→1.36x 확증. test_iobypass(+4) 콜드 동시 로딩 정확+A/B 동치, **ThreadSanitizer 클린**(축출 폭풍). ※ dirty 쓰기 I/O는 아직 latch 안·작은 풀·engine_mtx는 프론티어
