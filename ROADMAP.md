...
- [~] **H1. Primary-Replica 복제(log shipping)(25편)** — primary의 WAL을 replica가 tail하며 커밋된 after-image를 redo로 재적용(`src/replica.c`). redo가 idempotent라 lsn 필터로 체크포인트 truncate에도 견딤. **비동기 read replica까지 완료**(test_replica, 18 checks). TCP 전송(walsender/walreceiver)·복제 슬롯·동기 커밋은 프론티어로 남김
