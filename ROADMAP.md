...
- [~] **H2. 합의(Raft)(28편)** — `src/raft.c`: 리더 선출 + 로그 복제 + §5 안전성 5종(Election/Leader Append/Log Matching/Leader Completeness/State Machine Safety). 결정적 시뮬레이션 네트워크(분단·크래시·재정렬을 재현)로 검증 — test_raft 26 checks: 선출·복제·분단→새 리더→강등→로그 화해·선거 제약·크래시 복구. 스냅샷/멤버십 변경/디스크 지속성 배선/선형화 읽기는 프론티어(§6,§7)
